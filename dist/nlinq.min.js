...
b[k].push(e)});return b},countBy:(a,b,e)=>{a=_$(a).isString().default("key");b=_$(b).isString().default("_count");e=_$(e).isString().default(a);var k=c.group(a);return Object.keys(k).map(r=>{var q={};q[e]=r;q[b]=k[r].length;return q})},at:a=>{_$(a,"index").isNumber().$_();g=u(g);return g[Number(a)]},all:a=>{g=u(g);return $$(g).isArray()?g.length==h.length:a},count:()=>{g=u(g);return g.length},first:a=>{g=u(g);return 0<g.length?g[0]:a},last:a=>{g=u(g);return 0<g.length?g[g.length-1]:a},any:()=>{g=
...
c[a.selector.func].apply(c,a.selector.args)):b);$$(b).isUnDef()&&$$(a.select).isUnDef()&&(b=c.select());return b},select:a=>{g=u(g);if($$(a).isUnDef())return g;if($$(a).isFunction())return g.map(a);if($$(a).isArray()){var b={};a.forEach(k=>{$$(k).isString()&&$$(b).set(k,void 0)})}if($$(a).isMap()){var e=Object.keys(a);return g.map(k=>{var r={};e.forEach(q=>{$$($$(k).get(q)).isDef()?$$(r).set(q,$$(k).get(q)):$$(r).set(q,$$(a).get(q))});return r})}},mselect:(a,b,e)=>{a=c.select(a);b=_$(b,"aKey").isString().default("_key");
e=_$(e,"dontRemove").isBoolean().default(!0);var k={},r;for(r in a){var q=a[r];if($$(b).isDef()&&$$(q[b]).isDef()){var w=q[b];k[w]=q;e||delete k[w][b]}else k["row"+r]=q}return k},define:a=>{g=c.select(a);return c},removed:a=>{A=!0;g=c.select(a);return c},stream:a=>{var b=0<t?t:1;a=c.streamFn(a);do a(),0<t&&b--;while($$(g).isDef()&&0<b)},streamFn:a=>()=>{var b=c.select(a);g=$$(h).isFunction()?h():h;return b},query:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);
...
c[a.selector.func].apply(c,a.selector.args):b);$$(b).isUnDef()&&$$(a.select).isUnDef()&&(b=c.select());return b}};return c},$from=nLinq;
