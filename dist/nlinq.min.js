...
_$(b,"value").$_();g=u(g);g=$$(b).isFunction()?g.map(e=>{$$(e).set(a,b(e));return e}):g.map(e=>{$$(e).set(a,b);return e});return c},filter:a=>{_$(a,"value").$_();g=u(g);g=g.filter(b=>{var e=!0;$$(a).isMap()&&$$(b).isMap()&&Object.keys(a).forEach(k=>{E(a[k],b[k])||(e=!1)});$$(b).isArray()&&0>F(b,a)&&(e=!1);$$(a).isFunction()&&!a(b)&&(e=!1);($$(a).isNumber()||$$(a).isString())&&($$(b).isNumber()||$$(b).isString())&&b!=a&&(e=!1);return e});return c},sort:function(){var a="";g=u(g);for(var b=0;b<arguments.length;b++){var e=
arguments[b],k=!1;e.startsWith("-")&&(k=!0,e=e.substr(1,e.length-1));a=0<a.length?a+" || ":"return ";a=k?a+(' ($$(a).get("'+e+'") > $$(b).get("'+e+'") ? -1 : ($$(a).get("'+e+'") < $$(b).get("'+e+'") ? 1 : 0)) '):a+(' ($$(a).get("'+e+'") > $$(b).get("'+e+'") ? 1 : ($$(a).get("'+e+'") < $$(b).get("'+e+'") ? -1 : 0)) ')}g=g.sort(new Function("a","b",a));return c},assign:(a,b,e,k,q)=>{g=u(g);g.forEach(r=>{r[b]=nLinq(a).equals(k,$$(r).get(e)).first(q)});return c},join:(a,b,e,k)=>{g=u(g);g.forEach(q=>{q[b]=
nLinq(a).equals(k,$$(q).get(e)).select()});return c},skip:a=>{_$(a).isNumber().$_();x=a;return c},skipWhile:a=>{_$(a,"skip function").isFunction().$_();c.notWhere(a);return c},takeWhile:a=>{_$(a,"take function").isFunction().$_();c.where(a);return c},take:a=>{_$(a).isNumber().$_();return c.limit(a)},skipTake:(a,b)=>{_$(a).isNumber().$_();return c.skip(a).take(b)},apply:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);a.select=_$(a.select,"select").default(void 0);
a.transform=_$(a.transform,"transform").isArray().default([]);a.selector=_$(a.selector,"selector").isMap().default(void 0);a.where.forEach(e=>{$$(e.cond).isString()&&(c=c[e.cond].apply(c,e.args))});a.transform.forEach(e=>{$$(e.func).isString()&&(c=c[e.func].apply(c,e.args))});var b;$$(a.select).isString()&&(b=c.tselect(new Function("elem","index","array",a.select)));$$(a.select).isMap()&&(b=c.select(a.select));$$(b).isUnDef()&&$$(a.selector).isMap()&&(b=$$(a.selector.func).isString()?$$({}).set(a.selector.func,
c[a.selector.func].apply(c,a.selector.args)):b);$$(b).isUnDef()&&$$(a.select).isUnDef()&&(b=c.select());return b},select:a=>{g=u(g);if($$(a).isUnDef())return g;if($$(a).isFunction())return g.map(a);if($$(a).isArray()){var b={};a.forEach(k=>{if($$(k).isString())if(0<k.indexOf(":")){var q=k.substring(0,k.indexOf(":"));k=k.substring(k.indexOf(":")+1);b[q]=k}else b[k]=k});return g.map(k=>{var q={};Object.keys(b).forEach(r=>$$(q).set(r,$$(k).get(b[r])));return q})}if($$(a).isMap()){var e=Object.keys(a);
return g.map(k=>{var q={};e.forEach(r=>{$$($$(k).get(r)).isDef()?$$(q).set(r,$$(k).get(r)):$$(q).set(r,$$(a).get(r))});return q})}},mselect:(a,b,e)=>{a=c.select(a);b=_$(b,"aKey").isString().default("_key");e=_$(e,"dontRemove").isBoolean().default(!0);var k={},q;for(q in a){var r=a[q];if($$(b).isDef()&&$$(r[b]).isDef()){var w=r[b];k[w]=r;e||delete k[w][b]}else k["row"+q]=r}return k},define:a=>{g=c.select(a);return c},removed:a=>{A=!0;g=c.select(a);return c},stream:a=>{var b=0<t?t:1;a=c.streamFn(a);
do a(),0<t&&b--;while($$(g).isDef()&&0<b)},streamFn:a=>()=>{var b=c.select(a);g=$$(h).isFunction()?h():h;return b},query:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);a.select=_$(a.select,"select").default(void 0);a.transform=_$(a.transform,"transform").isArray().default([]);a.selector=_$(a.selector,"selector").isMap().default(void 0);a.where.forEach(e=>{$$(e.cond).isString()&&(c=c[e.cond].apply(c,e.args))});a.transform.forEach(e=>{$$(e.func).isString()&&
(c=c[e.func].apply(c,e.args))});var b;$$(a.select).isString()&&(b=c.tselect(newFn("elem","index","array",a.select)));$$(a.select).isMap()&&(b=c.select(a.select));$$(b).isUnDef()&&$$(a.selector).isMap()&&(b=$$(a.selector.func).isString()?c[a.selector.func].apply(c,a.selector.args):b);$$(b).isUnDef()&&$$(a.select).isUnDef()&&(b=c.select());return b}};return c},$from=nLinq;
