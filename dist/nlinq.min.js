/*v:0.1.5;b:Nuno Aguiar;Copyright 2023 Nuno Aguiar;*/
...
f=L(f,a);return c},except:a=>{_$(a,"except param").isArray().$_();f=v(f);f=K(f,a);return c},union:a=>{_$(a,"union param").isArray().$_();f=v(f);f=M(f,a);return c},cartesian:a=>{_$(a,"cartesian param").isArray().$_();f=v(f);f=N(f,a);return c},attach:(a,b)=>{_$(a,"key").$_();_$(b,"value").$_();f=v(f);f=$$(b).isFunction()?f.map(e=>{$$(e).set(a,b(e));return e}):f.map(e=>{$$(e).set(a,b);return e});return c},detach:a=>{_$(a,"key").$_();f=v(f);f=f.map(b=>{$$(b).unset(a);return b});return c},toDate:a=>{_$(a,
"key").$_();f=v(f);f.forEach(b=>{try{var e=$$(b).get(a);($$(e).isString()||$$(e).isNumber())&&$$(b).set(a,new Date(e))}catch(k){}});return c},filter:a=>{_$(a,"value").$_();f=v(f);f=f.filter(b=>{var e=!0;$$(a).isMap()&&$$(b).isMap()&&Object.keys(a).forEach(k=>{H(a[k],b[k])||(e=!1)});$$(b).isArray()&&0>J(b,a)&&(e=!1);$$(a).isFunction()&&!a(b)&&(e=!1);($$(a).isNumber()||$$(a).isString())&&($$(b).isNumber()||$$(b).isString())&&b!=a&&(e=!1);return e});return c},sort:function(){var a="";f=v(f);for(var b=
0;b<arguments.length;b++){var e=arguments[b],k=!1;e.startsWith("-")&&(k=!0,e=e.substr(1,e.length-1));a=0<a.length?a+" || ":"return ";a=k?a+(' ($$(a).get("'+e+'") > $$(b).get("'+e+'") ? -1 : ($$(a).get("'+e+'") < $$(b).get("'+e+'") ? 1 : 0)) '):a+(' ($$(a).get("'+e+'") > $$(b).get("'+e+'") ? 1 : ($$(a).get("'+e+'") < $$(b).get("'+e+'") ? -1 : 0)) ')}f=f.sort(new Function("a","b",a));return c},assign:(a,b,e,k,n)=>{f=v(f);f.forEach(r=>{r[b]=nLinq(a).equals(k,$$(r).get(e)).first(n)});return c},join:(a,
b,e,k)=>{f=v(f);f.forEach(n=>{n[b]=nLinq(a).equals(k,$$(n).get(e)).select()});return c},skip:a=>{_$(a).isNumber().$_();y=a;return c},skipWhile:a=>{_$(a,"skip function").isFunction().$_();c.notWhere(a);return c},takeWhile:a=>{_$(a,"take function").isFunction().$_();c.where(a);return c},take:a=>{_$(a).isNumber().$_();return c.limit(a)},skipTake:(a,b)=>{_$(a).isNumber().$_();return c.skip(a).take(b)},apply:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);a.select=
_$(a.select,"select").default(void 0);a.transform=_$(a.transform,"transform").isArray().default([]);a.selector=_$(a.selector,"selector").isMap().default(void 0);a.where.forEach(e=>{$$(e.cond).isString()&&(c=c[e.cond].apply(c,e.args))});a.transform.forEach(e=>{$$(e.func).isString()&&(c=c[e.func].apply(c,e.args))});var b;$$(a.select).isString()&&(b=c.tselect(new Function("elem","index","array",a.select)));$$(a.select).isMap()&&(b=c.select(a.select));$$(b).isUnDef()&&$$(a.selector).isMap()&&(b=$$(a.selector.func).isString()?
$$({}).set(a.selector.func,c[a.selector.func].apply(c,a.selector.args)):b);$$(b).isUnDef()&&$$(a.select).isUnDef()&&(b=c.select());return b},select:a=>{f=v(f,__,!0);if($$(a).isUnDef())return f;if($$(a).isFunction())return f.map(a);if($$(a).isArray()){var b={};a.forEach(k=>{if($$(k).isString())if(0<k.indexOf(":")){var n=k.substring(0,k.indexOf(":"));k=k.substring(k.indexOf(":")+1);b[n]=k}else b[k]=k});return f.map(k=>{var n={};Object.keys(b).forEach(r=>$$(n).set(r,$$(k).get(b[r])));return n})}if($$(a).isMap()){var e=
Object.keys(a);return f.map(k=>{var n={};e.forEach(r=>{$$($$(k).get(r)).isDef()?$$(n).set(r,$$(k).get(r)):$$(n).set(r,$$(a).get(r))});return n})}},mselect:(a,b,e)=>{a=c.select(a);b=_$(b,"aKey").isString().default("_key");e=_$(e,"dontRemove").isBoolean().default(!0);var k={},n;for(n in a){var r=a[n];if($$(b).isDef()&&$$(r[b]).isDef()){var u=r[b];k[u]=r;e||delete k[u][b]}else k["row"+n]=r}return k},define:a=>{f=c.select(a);return c},removed:a=>{C=!0;f=c.select(a);return c},stream:a=>{var b=0<t?t:1;
a=c.streamFn(a);do a(),0<t&&b--;while($$(f).isDef()&&0<b)},streamFn:a=>()=>{var b=c.select(a);f=$$(h).isFunction()?h():h;return b},query:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);a.select=_$(a.select,"select").default(void 0);a.transform=_$(a.transform,"transform").isArray().default([]);a.selector=_$(a.selector,"selector").isMap().default(void 0);a.where.forEach(e=>{$$(e.cond).isString()&&(c=c[e.cond].apply(c,e.args))});a.transform.forEach(e=>{$$(e.func).isString()&&
