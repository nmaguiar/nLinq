...
c.where(a);return c},take:a=>{_$(a).isNumber().$_();return c.limit(a)},skipTake:(a,b)=>{_$(a).isNumber().$_();return c.skip(a).take(b)},apply:a=>{a=_$(a,"aMap").isMap().default({});a.where=_$(a.where,"where").isArray().default([]);a.select=_$(a.select,"select").default(void 0);a.transform=_$(a.transform,"transform").isArray().default([]);a.selector=_$(a.selector,"selector").isMap().default(void 0);a.where.forEach(e=>{isString(e.cond)&&(c=c[e.cond].apply(c,e.args))});a.transform.forEach(e=>{isString(e.func)&&
(c=c[e.func].apply(c,e.args))});var b;isString(a.select)&&(b=c.tselect(new Function("elem","index","array",a.select)));isMap(a.select)&&(b=c.select(a.select));isUnDef(b)&&isMap(a.selector)&&(b=isString(a.selector.func)?$$({}).set(a.selector.func,c[a.selector.func].apply(c,a.selector.args)):b);isUnDef(b)&&isUnDef(a.select)&&(b=c.select());return b},select:a=>{g=u(g);if($$(a).isUnDef())return g;if($$(a).isFunction())return g.map(a);if($$(a).isArray()){var b={};a.forEach(k=>{$$(k).isString()&&$$(b).set(k,
void 0)})}if($$(a).isMap()){var e=Object.keys(a);return g.map(k=>{var r={};e.forEach(q=>{$$($$(k).get(q)).isDef()?$$(r).set(q,$$(k).get(q)):$$(r).set(q,$$(a).get(q))});return r})}},mselect:(a,b,e)=>{a=c.select(a);b=_$(b,"aKey").isString().default("_key");e=_$(e,"dontRemove").isBoolean().default(!0);var k={},r;for(r in a){var q=a[r];if($$(b).isDef()&&$$(q[b]).isDef()){var w=q[b];k[w]=q;e||delete k[w][b]}else k["row"+r]=q}return k},define:a=>{g=c.select(a);return c},removed:a=>{A=!0;g=c.select(a);return c},
stream:a=>{var b=0<t?t:1;a=c.streamFn(a);do a(),0<t&&b--;while($$(g).isDef()&&0<b)},streamFn:a=>()=>{var b=c.select(a);g=$$(h).isFunction()?h():h;return b}};return c},$from=nLinq;
